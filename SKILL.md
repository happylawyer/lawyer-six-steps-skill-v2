...
description: |
  中国律师办案六步法（事实梳理→法律分析→法理研究→案例检索→裁判规则→结果预判），并在最后生成完整案件分析报告。
  适用于律师对民商事案件做诉讼分析、要件审查、证据缺口识别、抗辩预判、胜诉概率评估。
  支持简易模式（3轮内给出可执行结论）与深度模式（六阶段逐步确认）。
  触发词：/lawyer-six-steps、/六步法、「案件分析」「办案分析」「诉讼预判」「要件审查」。
---
...
六步分析全部完成后，必须汇总生成一篇**完整案件分析报告**，写入文件并打开。
...
- 聊天界面只给简短提示，例如：`案件分析报告已整理完成，已写入并打开。`
- **不要在聊天界面完整展开整篇报告**
...
- 默认建议目录：`~/Library/Mobile Documents/iCloud~md~obsidian/Documents/OpenClawDoc/40-Outputs/Claude/`
